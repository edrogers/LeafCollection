...
targetDir="{}/Brush".format(dirName)
...
if prevStatus != currStatus:
    ...
    conn.request("POST", "/1/messages.json",
        urllib.urlencode({
            "token": config.apiKey,
            "user": config.apiUser,
            "message": "10-1 Brush Update: From \"{}\" to \"{}\"".format(prevStatus,currStatus),
...
if prevStatus != currStatus:
    ...
    conn.request("POST", "/1/messages.json",
        urllib.urlencode({
            "token": config.apiKey,
            "user": config.apiUser,
            "message": "8-99 Brush Update: From \"{}\" to \"{}\"".format(prevStatus,currStatus),
        }), { "Content-type": "application/x-www-form-urlencoded" })
    conn.getresponse()
