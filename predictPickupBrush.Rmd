...
madisonHolidays <- c(holidayNYSE(2015),timeDate("2015-11-27 05:00:00",format="%Y-%m-%d %H:%M:%S",FinCenter = "NewYork"),holidayNYSE(2016),timeDate("2016-11-25 05:00:00",format="%Y-%m-%d %H:%M:%S",FinCenter = "NewYork"))
...
# Restrict to 2016
leafData <- leafData[strftime(leafData$TimeStamps,"%Y") == 2016,]
```
...
generateModelData <- function(leafDataWest,targetArea) {
  ...
  modelData$status <- factor(leafDataWestStatuses[!afterLastPickup,targetArea],levels=c("Done","Current","Next","Recently Done","Not Done"))
  ...
  currentsInTargetArea <- leafDataWestStatuses[!afterLastPickup,targetArea]=="Current"
  ...
  # Allstreaks
  allStreakLength <- apply(leafDataWestStatuses[!afterLastPickup,]=="Current",2,conseq)+
                     apply(leafDataWestStatuses[!afterLastPickup,]=="Done",2,conseq)+
                     apply(leafDataWestStatuses[!afterLastPickup,]=="Next",2,conseq)
...
#Proposed fits:
(fit <- train(nDaysTilPickup ~ nColsFromCurrent + status + nCurrent + status*nCurrent,data=modelData,method="lm",trControl=tc))
(fit2 <- train(nDaysTilPickup ~ nColsFromCurrent + status + nCurrent + status*nCurrent + nColsFromCurrent*status,data=modelData,method="lm",trControl=tc))
(fit3 <- train(nDaysTilPickup ~ nColsFromCurrent + status + nCurrent + status*nCurrent + nColsFromCurrent*status + Area,data=modelData,method="lm",trControl=tc))
# (fit4 <- train(nDaysTilPickup ~ nColsFromCurrent + status + nCurrent + status*nCurrent + nColsFromCurrent*status + Area + Area*status,data=modelData,method="lm",trControl=tc))
(fit5 <- train(nDaysTilPickup ~ nColsFromCurrent + status + nCurrent + status*nCurrent + nColsFromCurrent*status + Area + ageOfStatus,data=modelData,method="lm",trControl=tc))
(fit6 <- train(nDaysTilPickup ~ nColsFromCurrent + status + nCurrent + status*nCurrent + nColsFromCurrent*status + Area + ageOfStatus + ageOfStatus*ageOfStatus,data=modelData,method="lm",trControl=tc))
(fit7 <- train(nDaysTilPickup ~ nColsFromCurrent + status + nCurrent + status*nCurrent + nColsFromCurrent*status + Area + ageOfStatusGT10,data=modelData,method="lm",trControl=tc))

# #fit3 is best
# selectedFit <- lm(nDaysTilPickup ~ nColsFromCurrent + status + nCurrent + status*nCurrent + nColsFromCurrent*status + Area,data=modelData)
#fit6 is best
selectedFit <- lm(nDaysTilPickup ~ nColsFromCurrent + status + nCurrent + status*nCurrent + nColsFromCurrent*status + Area + ageOfStatusGT10,data=modelData)

dataForPlots <- data.frame(modelData,fitted=selectedFit$fitted.values,residuals=selectedFit$residuals)
# }

saveRDS(selectedFit, "modelBrush.rds", ascii=TRUE)
```
...
ggplot(data=dataForPlots) + geom_jitter(aes(x=nDaysTilPickup,y=residuals,colour=ageOfStatus),size=2,alpha=0.7)
```
