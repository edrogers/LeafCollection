...
#  is not equal to "Current", but immediately follows a "Current".
#  Each time this happens a row is marked true. Then, a cumsum is 
#  taken over that column, which gives a column that
#  is iterated if-and-only-if a "Current" status expires.
leafData <- leafData %>% 
            mutate_each_(funs(cumsum(as.numeric(. != "Current") * as.numeric(head(c("Done",.),-1) == "Current"))),leafDataCollectionRoundNames) 
...
#  but not followed by a "Current" in the next row.
#   (A dummy function must be used in addition just to get 
#   mutate_each to behave. This set of columns, "..._sd" is
#   immediately discarded)
leafData <- leafData %>%
              mutate_each(funs(nBusinessDaysTilPickup=(as.numeric(!((leafData$Area10_001 == "Current") & (tail(c(leafData$Area10_001,"Current"),-1) != "Current")))*BusinessDaysTilNextEntry),sd=is.na),starts_with("Area"))
leafData <- leafData %>%
  select(-contains("_sd"))
