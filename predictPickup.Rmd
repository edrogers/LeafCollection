...
### Was testing out using rle to find the beginning and end
### of the wave of "Done" statuses rolling through town, when
### I realized I could probably get away with something much
### simpler: rowSums(status=="Current")
# # Find first and last "Current" column location, given that 
# #  the block of "Current"s might be broken up to across the
# #  beginning and end of the data.frame
# listRunsInStatuses <- apply(leafDataWestStatuses,1,rle)
# lapply(listRunsInStatuses,function (x) cumsum(x$length)[x$values=="Done" & x$length==max(x$length[x$values=="Done"])])
...
altModelData <- modelData[modelData$status!="Recently Done",]
fit3 <- lm(nDaysTilPickup ~ ., data=altModelData)

altModelData2 <- modelData[modelData$status=="Next" | modelData$status=="Done",]
fit4 <- lm(nDaysTilPickup ~ nColsFromCurrent + status + nColsFromCurrent*status, data=altModelData2)
fit4 <- lm(nDaysTilPickup ~ ., data=altModelData2)

coefficients2 <- coef(fit2)
coefficients3 <- coef(fit3)
coefficients4 <- coef(fit4)

ggplot(data=modelData,aes(x=nColsFromCurrent,y=nDaysTilPickup)) +
geom_jitter(aes(colour=factor(status)),size=3) +
#stat_smooth(method="lm",col="blue",fill="lightblue",formula = y ~ x) +
  geom_abline(aes(intercept=coefficients4[1],slope=coefficients4[2]),color="red") +
  geom_abline(aes(intercept=coefficients4[1]+coefficients4[3],slope=coefficients4[2]),color="cyan")
#   geom_abline(aes(intercept=coefficients2[1]+coefficients2[4],slope=coefficients2[2]),color="blue") +
#   geom_abline(aes(intercept=coefficients2[1]+coefficients2[5],slope=coefficients2[2]),color="purple")

# ggplot(data=modelData,aes(x=nColsFromCurrent,y=nDaysTilPickup)) +
# geom_jitter(aes(colour=factor(status)),size=3) +
# #stat_smooth(method="lm",col="blue",fill="lightblue",formula = y ~ x) +
#   geom_abline(aes(intercept=coefficients3[1],slope=coefficients3[2]),color="red") +
#   geom_abline(aes(intercept=coefficients3[1]+coefficients3[3],slope=coefficients3[2]),color="green") +
#   geom_abline(aes(intercept=coefficients3[1]+coefficients3[4],slope=coefficients3[2]),color="blue")
